...
class ArrowSecondClockView @JvmOverloads constructor(
    context: Context,
    ...
    private var specialNumberPaint: Paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        textSize = 86f
        color = Color.GRAY
        typeface = resources.getFont(R.font.nmc_regular)
    }
    ...
    private fun drawClockHands(canvas: Canvas) {
        ...
        val minuteHandLength = radius * 0.78
        canvas.drawLine(centerX, centerY,
        ...
        val secondHandLength = radius * 0.96
        canvas.drawLine(centerX, centerY,
    ...
    private fun drawClockFace(canvas: Canvas) {
        val centerX = width / 2f
        val centerY = height / 2f
        val radius = width.coerceAtMost(height) / 2f * 0.86f // Slightly reduce radius for aesthetics
